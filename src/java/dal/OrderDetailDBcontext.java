...
//    }
    ...
    public List<OrderDetail> getOrderDetailsSortedByTotalPrice() {
        List<OrderDetail> orderDetails = new ArrayList<>();
        try {
            String sql = "SELECT * FROM [dbo].[OrderDetail] ORDER BY [quantity] * [productPrice] DESC";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                ...
                orderDetail.setProductPrice(rs.getFloat("productPrice"));
      ...
      public static void main(String[] args) {
        OrderDetailDBcontext test = new OrderDetailDBcontext();
        List<OrderDetail> orderDetails = test.getOrderDetailsSortedByTotalPrice();
